...
- `PixelSwitch` — pixel-styled boolean toggle with sliding thumb animation, `Switch`-role semantics (toggled state), and `Space`/`Enter` keyboard support. Slots: `onTrackStyle`, `offTrackStyle`, `thumbStyle`, optional `disabledStyle`. `enabled == false` independent of `onChanged != null` — paints `disabledStyle` if given else 50% opacity. New `PixelSwitchTheme` extension (also slotted on `PixelTheme.switch_` and `pixelUiTheme(switchTheme:)`) lets apps register defaults. Tuner integration deferred to a follow-up issue. Closes #47.
- `PixelSlider` — pixel-styled value slider (continuous or discrete) with sliding thumb, drag/tap input, optional `divisions` snapping, slider-role semantics with optional `semanticsValueText` formatter, and Arrow / PageUp / PageDown keyboard support. Slots: `trackStyle`, `fillStyle`, `thumbStyle`, optional `disabledStyle`. `enabled == false` independent of `onChanged != null` — paints `disabledStyle` (track only) if given, else wraps the visual at 50% opacity. New `PixelSliderTheme` extension (also slotted on `PixelTheme.slider` and `pixelUiTheme(sliderTheme:)`) lets apps register defaults. Tuner integration deferred to a follow-up issue. Closes #48.
